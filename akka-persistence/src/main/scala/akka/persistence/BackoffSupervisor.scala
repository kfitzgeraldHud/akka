...
object BackoffSupervisor {
  ...
  /**
   * Send this message to the [[BackoffSupervisor]] and it will reply with [[CurrentChild]]
   * containing the `ActorRef` of the current child, if any.
   */
  final case object GetCurrentChild

  /**
   * Java API: Send this message to the [[BackoffSupervisor]] and it will reply with [[CurrentChild]]
   * containing the `ActorRef` of the current child, if any.
   */
...
final class BackoffSupervisor(
  ...
  def receive = {
    case Terminated(ref) if child.contains(ref) ⇒
      ...
      val restartDelay = maxBackoff.min(minBackoff * math.pow(2, restartCount)) * rnd
      restartCount += 1
